...
resource "null_resource" "deploy" {
  ...
  provisioner "local-exec" {
    command = <<EOT
      az login --service-principal -u ${var.artifacts.client_id} -p ${var.artifacts.client_secret} --tenant ${var.artifacts.tenant_id} --output json
      az account set --subscription ${var.artifacts.subscription} --output json
